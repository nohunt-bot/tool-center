...
describe("resolveRole：角色相對於課別", () => {
  it("該課 manager → admin", () => {
    expect(resolveRole(MANAGER, "LITHO-02", [], NOW)).toBe("admin");
  });

  it("該課工程師 → editor", () => {
    expect(resolveRole(LAO_LI, "LITHO-02", [], NOW)).toBe("editor");
  });

  it("同一人切到別課 → viewer（這是模型的重點）", () => {
    expect(resolveRole(MANAGER, "LITHO-02", [], NOW)).toBe("admin");
    expect(resolveRole(MANAGER, "ETCH-01", [], NOW)).toBe("viewer");
  });

  it("跨課支援的工程師 → viewer", () => {
    expect(resolveRole(LAO_LI, "LITHO-01", [], NOW)).toBe("viewer");
  });

  it("sponsor 預設 viewer，但可寫 Tool Command（部分放行）", () => {
    const role = resolveRole(SPONSOR, "LITHO-02", [], NOW);
...
describe("grant：admin 授予課外人員 editor", () => {
  it("授予後升為 editor", () => {
    expect(resolveRole(SPONSOR, "LITHO-02", [grant()], NOW)).toBe("editor");
  });

  it("撤銷（grant 移除）後降回 viewer", () => {
    expect(resolveRole(SPONSOR, "LITHO-02", [], NOW)).toBe("viewer");
  });

  it("expiresAt 過期後自動失效", () => {
    const expired = grant({ expiresAt: new Date("2026-07-23T00:00:00Z") });
    expect(resolveRole(SPONSOR, "LITHO-02", [expired], NOW)).toBe("viewer");
  });

  it("未過期的時效性 grant 仍有效", () => {
    const active = grant({ expiresAt: new Date("2026-07-25T00:00:00Z") });
    expect(resolveRole(SPONSOR, "LITHO-02", [active], NOW)).toBe("editor");
  });

  it("grant 只對指定課別生效，不會外溢到其他課", () => {
    expect(resolveRole(SPONSOR, "ETCH-01", [grant()], NOW)).toBe("viewer");
  });

  it("grant 授予後仍不能改專家標籤與移除 KM 來源（那是 admin-only）", () => {
    const role = resolveRole(SPONSOR, "LITHO-02", [grant()], NOW);
...
describe("canEnterSection：跨課需支援權限", () => {
  it("自己的課、管理的課、有支援權限的課都能進", () => {
    expect(canEnterSection(LAO_LI, "LITHO-02")).toBe(true);
    expect(canEnterSection(MANAGER, "LITHO-02")).toBe(true);
    expect(canEnterSection(LAO_LI, "LITHO-01")).toBe(true);
  });

  it("沒有支援權限的課別進不去", () => {
    expect(canEnterSection(LAO_LI, "ETCH-01")).toBe(false);
  });
});
